...
from telegram_bot.template.telegram_channel_book_post import (
    CHANNEL_POST_BODY,IN_STOCK_TEXT,OUT_OF_STOCK_TEXT,ORDER_NOW_BTN_TEXT,
    ADD_TO_WISHLIST_BTN_TEXT,REVIEWS_BTN_TEXT,RATE_BTN_TEXT
)
...
class PostController:
    ...
    CHANNEL_USERNAME = CONFIG.get("telegram_api","CHANNEL_USERNAME")\
                    .strip('"')
        ...
        for round_number in range(total_round):
            ...
            for book in updated_books:
                ...
                loop.run_until_complete(
                    self.edit_post_from_channel(post_id,post_data))

                # changing book content status from updated-> original
                self.post_model.update_book_content_status(book_code)
    ...
    async def send_post_to_channel(self, book_code, message):
        ...
        response_msg = await Bot.send_message(chat_id=self.CHANNEL_USERNAME,
            text=message,reply_markup=post_inline_btn)
    ...
    async def edit_post_from_channel(self, post_id, message):
        await Bot.edit_message_text(chat_id=self.CHANNEL_USERNAME,
            message_id=post_id,text=message)
    ...
    async def post_inline_btn(self, book_code):
        ...
        order_now_btn = InlineKeyboardButton(
                text=ORDER_NOW_BTN_TEXT,
                url=order_now_btn_link
        )

        add_to_wishlist_btn_link = await create_start_link(
            bot=Bot,
            payload=f"wishlist_btn&book_code={book_code}",
            encode=True
        )
        add_to_wishlist_btn = InlineKeyboardButton(
                text=ADD_TO_WISHLIST_BTN_TEXT,
                url=add_to_wishlist_btn_link
        )

        reviews_btn_link = await create_start_link(
            bot=Bot,
            payload=f"reviews_btn&book_code={book_code}",
            encode=True
        )
        reviews_btn = InlineKeyboardButton(
                text=REVIEWS_BTN_TEXT,
                url=reviews_btn_link
        )
    ...
    def post_template(self, data):
        ...
        for category_name_part in category_name_parts:
            ...
            hashtag_category_names += "#"+category_name_part.strip()\
                                        .replace(" ","_")+" "
